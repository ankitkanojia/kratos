...
contract('KratosToken', function (accounts) {
    ...
    it('initializes the contract with the correct value', function () {
        return KratosToken.deployed().then(function (instance) {
            ...
            assert.equal(name, 'Kratos Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function (symbol) {
            assert.equal(symbol, 'Kratos', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function (standard) {
            assert.equal(standard, 'Kratos Token v1.0', 'has the correct standard');
        });
    });
